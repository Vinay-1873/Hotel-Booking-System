...
const ListRoom = () => {
  ...
  // fetch rooms listed by hotel owner
  const fetchRooms = async () => {
    try {
      // server exposes owner rooms at /api/rooms/owner
  ...
  const toggleAvailability = async (roomId) => {
    // API call to toggle availability
  ...
  return (
    <div>
     ...
     keep the information up-to-date to 
              ...
              {rooms.map((item,index)=>(
                <tr key={index}>
                  ...
                  <td className='py-3 px-4 text-gray-700 border-t border-gray-300 max-sm:hidden'>
                    {Array.isArray(item.amenities) ? item.amenities.join(', ') : (item.amenities || '').toString()}
                  </td>
                  <td className='py-3 px-4 text-gray-700 border-t border-gray-300'>
                    {currency} {item.pricePerNight ?? item.priceperNight ?? item.price}
                  </td>
